...
for model in "${models[@]}"; do
    for scheduler in "cosine"; do
        echo "=========================================="
        echo "Training model: $model with scheduler: $scheduler"
        echo "=========================================="
        python train.py --config config/${model}_adamw.json --dataset fashionmnist --input_size 28 --batch_size 64 --use_tensorboard --tensorboard_log_dir results/tensorboard/${model}_${scheduler}_fashionmnist --use_wandb --wandb_project dl20251-cv-new --wandb_run_name ${model}_${scheduler}_fashionmnist --scheduler $scheduler --num_epochs 25
